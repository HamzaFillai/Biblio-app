...
{
	$del=$bdd->prepare("DELETE FROM livre WHERE isbn = ?");
...
if(isset($_GET["edit"]))
{
	$ress=$bdd->prepare("SELECT * FROM livre WHERE isbn = ?");
	...
	$prix=$aff["prix"];
	$nbrpage=$aff["nbrpage"];
	$qtedispo=$aff["qtedisponible"];
	$qtetotal=$aff["qtetotal"];
	$date=$aff["anneeparution"];
	$resume=$aff["resume"];
	$supp=$bdd->prepare("DELETE FROM livre WHERE isbn = ?");
...
<body>
	...
	<div class="section">
		...
		<h1><i class="fas fa-book-open"></i> Mes livres <i class="fas fa-book-open"></i></h1>
		<br/>
		<p style="font-weight: bold;font-size: 18px; "><i class="fas fa-exclamation-triangle fa-2x" style="color: red;"></i>Si vous voulez modfié un livre, vous cliquez sur l'icon qui est en bleu après vous cliquez sur ajoutez livre et comme ça votre livre sera modifié</p>
					...
					while($don=$res->fetch())
						{?>
							<tr>
								...
								<td><?php echo $don["auteur"]; ?></td>
								<td><?php echo $don["prix"]; ?></td>
								<td><?php echo $don["qtedisponible"];?></td>
								<td><?php echo $don["qtetotal"]; ?></td>
								<td >
									<a style="color: red;" href="listlivre.php?delete=<?php echo $don["isbn"]; ?>"><i class="fas fa-trash-alt"></i></a> <span style="opacity: 0">hh</span>
									<a  href="listlivre.php?edit=<?php echo $don["isbn"]; ?>"><i class="fas fa-pen-alt"></i></a>
								</td>
							</tr>
						<?php
						}
	...
	<div class="modal-bg">
		<form class="modal" action="listlivre.php" method="post">
			...
			<p>
				...
				<?php
				 ...
				 {?>
				 	...
				 	<script>swal('Good!', 'This book has been added !', 'success');setTimeout('window.location.href = "listlivre.php";',2000);</script>
				 <?php	
				 }
				?>
			</p>
			<table>
				<tr>
					...
					<td><input class="inp1" type="text" name="titre" required="" value=<?php if(isset($titre)) echo $titre;?>></td>
				</tr>
				<tr>
					<td align="right"><label>Auteur : </label></td>
					<td><input class="inp1" type="text" name="auteur" required="" value=<?php if(isset($auteur)) echo $auteur;?>></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><input class="inp1" type="text" name="genre" required="" value=<?php if(isset($genre)) echo $genre;?>></td>
				</tr>
				<tr>
					<td align="right"><label>Editeur : </label></td>
					<td><input class="inp1" type="text" name="editeur" required="" value=<?php if(isset($editeur)) echo $editeur;?>></td>
				</tr>
				<tr>
					<td align="right"><label>Prix : </label></td>
					<td><input class="inp1" type="text" name="prix" required="" value=<?php if(isset($prix)) echo $prix;?>></td>
				</tr>
				<tr>
					<td align="right"><label>Nombre de pages : </label></td>
					<td><input class="inp1" type="text" name="page" required="" value=<?php if(isset($nbrpage)) echo $nbrpage;?>></td>
				</tr>
				<tr>
					<td align="right"><label>Quantité totale : </label></td>
					<td><input class="inp1" type="text" name="qtetotal" required="" value=<?php if(isset($qtetotal)) echo $qtetotal;?>></td>
				</tr>
				<tr>
					<td align="right"><label>Quantité disponibile : </label></td>
					<td><input class="inp1" type="text" name="qtedispo" required="" value=<?php if(isset($qtedispo)) echo $qtedispo;?>></td>
				</tr>
				<tr>
					<td align="right"><label>Anneé de parution : </label></td>
					<td><input class="inp1" type="date" name="date" required="" value=<?php if(isset($date)) echo $date;?>></td>
				</tr>
				<tr>
					<td align="right"><label>Résumé : </label></td>
					<td><textarea name="resume" required="" class="inp2"><?php if(isset($resume)) echo $resume;?></textarea></td>
				</tr>
				<tr hidden>
	...
	<div class="ajoutadh">
		<form class="formadh" action="listlivre.php" method="post">
	...
	<div class="ajoutb">
		<form class="formb" action="listlivre.php" method="post">
	...
	<div class="modp">
		<form class="formp" action="listlivre.php" method="post">
